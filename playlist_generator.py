...
def main():
    ...
    listening_habits_periods = {period:listening_habits.get(period, None) for period in [current_hour]}

    if listening_habits_periods and any(value is not None for value in listening_habits_periods.values()): 
        listening_history_filtered = {hour:songs for hour, songs in listening_history.items() if hour == current_hour}
            ...
            if clusterings:
                ...
                if most_similar_song_sets:
                    ...
                    if history_patterns:
                        ...
                        playlists_rec_1 = other_methods.get_rec_1_recommendations(listening_history_filtered,prefix_name, current_day)
                        playlists_rec_2 = other_methods.get_rec_2_recommendations(listening_history_filtered,prefix_name, current_day)
                        playlists_hyb_1, best_history_patterns = other_methods.get_hyb_1_recommendations(listening_history_filtered, periods,prefix_name, current_day)
                        ...
                        if all_playlists:
                            evaluation.compute_playlist_pattern_distances(all_playlists, results)
                        else:
                        ...
                        print(f"No history patterns found for hour {current_hour} on day {current_day}")
            else:
                 print(f"No valid clusterings produced for hour {current_hour}")
        else:
            print(f"No listening history detected for hour {current_hour}")
    else:
        print(f"No habits detected for hour {current_hour}")
